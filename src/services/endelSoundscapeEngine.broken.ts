...
// Endel-style soundscap    duration: 10,
...
export const ENDEL_SOUNDSCAPES = {
  focus: {
    ...
    tracks: [
      { name: 'Deep Focus', intensity: 'high', duration: 3600, url: 'https://www.soundjay.com/misc/sounds/bell-ringing-05.wav' },
      { name: 'Gentle Focus', intensity: 'medium', duration: 2400, url: 'https://www.soundjay.com/misc/sounds/bell-ringing-05.wav' },
      { name: 'Flow State', intensity: 'adaptive', duration: 5400, url: 'https://www.soundjay.com/misc/sounds/bell-ringing-05.wav' }
    ]
  },
  relax: {
    id: 'relax',
    ...
    musicUrl: 'https://www.soundjay.com/nature/sounds/water-wave-1.wav',
    tracks: [
      { name: 'Deep Calm', intensity: 'low', duration: 1800, url: 'https://www.soundjay.com/nature/sounds/water-wave-1.wav' },
      { name: 'Gentle Waves', intensity: 'medium', duration: 2700, url: 'https://www.soundjay.com/nature/sounds/water-wave-1.wav' },
      { name: 'Peaceful Sanctuary', intensity: 'adaptive', duration: 3600, url: 'https://www.soundjay.com/nature/sounds/water-wave-1.wav' }
    ]
  },
  sleep: {
    id: 'sleep',
    ...
    musicUrl: 'https://www.soundjay.com/nature/sounds/rain-02.wav',
    tracks: [
      { name: 'Sleep Induction', intensity: 'very_low', duration: 2700, url: 'https://www.soundjay.com/nature/sounds/rain-02.wav' },
      { name: 'Deep Sleep', intensity: 'minimal', duration: 28800, url: 'https://www.soundjay.com/nature/sounds/rain-02.wav' },
      { name: 'Dawn Transition', intensity: 'gentle', duration: 1800, url: 'https://www.soundjay.com/nature/sounds/rain-02.wav' }
    ]
  },
  activity: {
    id: 'activity',
    ...
    musicUrl: 'https://www.soundjay.com/nature/sounds/wind-blow-1.wav',
    tracks: [
      { name: 'Walking Meditation', intensity: 'rhythmic', duration: 3600, url: 'https://www.soundjay.com/nature/sounds/wind-blow-1.wav' },
      { name: 'Mindful Movement', intensity: 'flowing', duration: 2400, url: 'https://www.soundjay.com/nature/sounds/wind-blow-1.wav' },
      { name: 'Grounding Steps', intensity: 'steady', duration: 1800, url: 'https://www.soundjay.com/nature/sounds/wind-blow-1.wav' }
    ]
  }
};

// Scenarios (timed sessions with specific purposes)
...
export class EndelSoundscapeEngine {
  ...
  private async setupAudio() {
    try {
      ...
      console.log('🎵 Endel audio system initialized');
    } catch (error) {
      console.error('❌ Endel audio setup failed:', error);
    }
  }
  ...
  enableAutoplay() {
    ...
    console.log('🤖 Endel Autoplay enabled');
  ...
  disableAutoplay() {
    ...
    console.log('🤖 Endel Autoplay disabled');
  }

  // Adaptive soundscape playback
  async playSoundscape(soundscapeId: string): Promise<boolean> {
    try {
      ...
      console.log(`🎵 Playing Endel ${soundscape.name}: ${selectedTrack.name}`);
      console.log(`📊 Adapted for: ${soundscape.adaptiveFactors.join(', ')}`);

      // Load and play the actual music
      if (soundscape.musicUrl) {
        ...
        console.log(`🎶 Audio loaded and playing with loop: ${soundscape.musicUrl}`);
      }
  ...
  async playScenario(scenarioId: string): Promise<boolean> {
    try {
      ...
      console.log(`🎯 Starting Endel Scenario: ${scenario.name}`);
  ...
  async pauseSoundscape(): Promise<void> {
    try {
      ...
      this.isPlaying = false;
      console.log('⏸️ Endel soundscape paused');
    } catch (error) {
  ...
  async resumeSoundscape(): Promise<void> {
    try {
      ...
      this.isPlaying = true;
      console.log('▶️ Endel soundscape resumed');
    } catch (error) {
  ...
  async stopAll(): Promise<void> {
    try {
      ...
      this.currentScenario = null;
      this.isPlaying = false;
      console.log('🛑 All Endel audio stopped');
    } catch (error) {
